...
def get_route_parallelism(cities, network):
    ...
    with Pool(8) as p:
        ...
        input = [(i,network) for i in cities]
        print('lenght of input: ',len(input))
        route = p.starmap(compare_node_with_weight_matrix, input, 10)
...
network = np.load('weighted_network_4.npz').get('network')
...
if __name__ == '__main__':
    ...
    route = get_route(cities[1:25], network)
